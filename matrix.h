...
{
	if (this->length != p.length || width != p.width) throw "err";
	TMatrix<T> res(*this);
	for (int i = 0; i < width; i++)
...
inline TMatrix<T> TMatrix<T>::operator*(const TMatrix<T>& p)
{
	if (this->length != p.width) throw "err";
...
{
	if (this->length != p.GetLen()) throw "err";
...
{
	std::cout << "¬ведите " << A.width * A.length << " размеры матрицы: \n";
	for (int i = 0; i < A.width; i++)
		for (int j = 0; j < A.length; j++)
			B >> A.data[i][j];
	return B;
...
{
	if (matrix.GetWidth() != 1) throw "err";
